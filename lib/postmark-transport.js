...
var addressFormatter = utils.addressFormatter;
var addressParser = utils.addressParser;
var headersParser = utils.headersParser;
...
PostmarkTransport.prototype._parse = function (mails, callback) {
  ...
  function mapper(mail, next) {
    data = mail.data || mail;
    fromAddr = addressParser(data.from)[0] || {};
    toAddrs = addressParser(data.to) || [];
    ccAddrs = addressParser(data.cc) || [];
    bccAddrs = addressParser(data.bcc) || [];
    headers = headersParser(data.headers || []);
...
PostmarkTransport.prototype._parseAttachments = function (attachments, callback) {
  ...
  var elements = mailComposer._getAttachments().attached;
  var options = {
    lineLength: false
  };
  var attachment;
