...
function parser(mail, next) {
  ...
  this._parseAttachments(data.attachments || [], (err, attachments) => {
    ...
    if (typeof data.templateAlias === 'undefined' && typeof data.templateId === 'undefined') {
      message.setSubject(data.subject);
      message.setHtmlBody(data.html);
      message.setTextBody(data.text);
    } else {
      message.setTemplate(data.templateId, data.templateModel, data.inlineCss, data.templateAlias);
    }
...
function PostmarkTransport(options) {
  ...
  this.client = new postmark.ServerClient(auth.apiKey);
}
...
PostmarkTransport.prototype.send = function (mail, callback) {
  this._parse(mail, (err, message) => {
    ...
    const method = (typeof message.TemplateId === 'undefined' && typeof message.TemplateAlias === 'undefined') ?
      'sendEmail' :
